        ...
        if (installed) {
          this.adb.getVersions(this.args.appPackage, function (err, versions) {
            this.adb.getVersionFromManifest(this.args.app, function (err, version) {
              ...
              logger.debug('Installed versions: ' + toString(versions));
              logger.debug('Test version: ' + toString(version));
              if (installed) {
                install(installed, remoteApk);
              } else {
                this.adb.uninstallApk(this.args.appPackage, function (err) {
                  if (err) return cb(err);
                  install(false, remoteApk);
                }.bind(this));
              }
            }.bind(this));
          }.bind(this));
...
androidCommon.broadcastIntent = function (intent, receiver, keys, cb) {
  this.adb.broadcastIntent(intent, this.args.appPackage, receiver, keys, function (err) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
...
androidCommon.setDate = function (time, cb) {
  this.adb.setDate(time, function (err, shown) {
    if (err) return cb(err);
    cb(null, {
      status: status.codes.Success.code
    });
