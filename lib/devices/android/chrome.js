...
ChromeAndroid.prototype.configure = function (args, caps, cb) {
  ...
  } else if (app === "yandex-browser") {
    this.args.androidPackage = "com.yandex.browser";
    this.args.androidActivity = "com.yandex.browser.YandexBrowserActivity";
  } else {
...
ChromeAndroid.prototype.createSession = function (cb) {
  ...
  new Q(this.chromedriver.start(caps)).then(function () {
    return this.chromedriver.sessionId();
...
ChromeAndroid.prototype.stop = function (cb) {
  ...
  new Q(this.chromedriver.stop())
    .catch(function (err) { logger.warn("Error stopping Chromedriver: " + err.message); })
    .finally(function () {
      this.onChromedriverStop(cb); // TODO: do not rely on class method for callback
    }.bind(this)).done();
};

ChromeAndroid.prototype.onChromedriverStop = function (cb) {
  var killAVD = function () {
