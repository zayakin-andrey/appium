...
var errors = require('../../server/errors.js')
  ...
  , grid = require('../../server/grid-register.js')
  , async = require('async')
...
Android.prototype.restartUiautomator = function (cb) {
  var tasks = [this.forwardPort.bind(this)];
  if (this.uiautomator) {
    tasks.push(this.uiautomator.start.bind(this.uiautomator));
    tasks.push(this.setupCompressedLayoutHierarchy.bind(this));
  }
...
Android.prototype.pushStrings = function (cb, language) {
  ...
  this.extractStrings(function (err, localPath) {
    if (err) {
...
Android.prototype.startApp = function (args, cb) {
  if (args.androidCoverage) {
    this.adb.androidCoverage(args.androidCoverage, args.appWaitPackage, args.appWaitActivity, cb);
  } else {
    this.adb.startApp({
      pkg: args.appPackage,
      ...
      waitActivity: args.skipActivityWaiting ? null : args.appWaitActivity,
...
Android.prototype.stop = function (cb) {
  ...
  if (!this.adb && !this.uiautomator) {
    logger.debug("Already shutted down.");
    return cb();
  }
