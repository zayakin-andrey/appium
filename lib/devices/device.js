...
Device.prototype.deleteTempFiles = function (cb) {
  logger.info('Delete downloaded files');
  var root = os.tmpDir();
  var i = 0;
  while (this.tempFiles[i]) {
    var dir = this.tempFiles[i].replace(new RegExp("^(" + root + "/[^/]+)/.+$"), "$1");
    rimraf.sync(dir);
